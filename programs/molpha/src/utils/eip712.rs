...
pub fn get_data_source_type_hash() -> [u8; 32] {
    keccak::hashv(&[
        b"DataSource(uint8 type,string source,address owner,string name)"
    ]).to_bytes()
}
...
/// Build the struct hash for DataSource - match Solidity contract exactly
pub fn build_struct_hash(data: &crate::state::DataSourceData) -> [u8; 32] {
...
pub fn compute_data_source_id(data: &crate::state::DataSourceData) -> Result<[u8; 32]> {
